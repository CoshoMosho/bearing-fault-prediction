...
'''
Simple one layer CNN. Depends a lot on its size.
'''
...
def create_model(train_data, train_target, val_data, val_target, params):
    ...
    checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
        filepath='tmp/checkpoint.weights.h5',  # <--- AGGIUNTO .weights.h5
    ...
    model.compile(
        ...
        optimizer='adam',
        metrics=['accuracy']  # <--- LISTA, non stringa!
    )
    ...
    model.load_weights('tmp/checkpoint.weights.h5')  # <--- ESTENSIONE ESPLICITA
